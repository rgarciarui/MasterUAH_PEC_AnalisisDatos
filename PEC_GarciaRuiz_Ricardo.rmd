...
Esta accion se puede abordar de diferente maneras, incluyendo el uso de la librería **dplyr**. Por ejemplo con '**bikes %>% map(~ mean(is.na(.)))**'. No obstante, vamos a usar una variante que nos permite su representación en forma de tabla:
...
En este paso volvemos a proceder a la agrupación por distrito y posteriormente se aplica la función **mutate()** para aplicar la media a calculada para cada distrito y volcarla solo a los valores NA.
...
A continuación, seleccionamos únicamente la variable **Survived** y realizamos un sencillo cálculo:
...
Realizamos el cálculo del porcentaje de valores 'NA' en cada una de las variables, aunque solo ser´ñia necesario en 3 que son las únicas que tienen valores ausentes:
...
1. Separamos la variable original **Name** en dos nuevas variables usando de token separador la ',' que separa nombre de apellidos.
2. Separamos nuevamente ahora esta variable 'intermedia' denominada **Nombre** en 2 nuevas variables, la segunda de las cuales ya contiene el conjunto de datos de titulo.
3. Descartamos las variables intermedias que han resultado del proceso de separación y que no aportan nada al resultado.
4. Realizamos la limpieza de la nueva variable **Title** ajustando los titulos que no son correctos con los aceptados y, finalmente, el resto de datos no ajustables se sustituyen por la palabra 'Otros'.
...
* **Mediana**: En las variables 'Master', 'Miss', 'Mr.' y 'Mrs' la linea de caja esta un poco por debajo de la zona central de caja. En la variable 'Otro' dicha linea de caja esta muy desplazada a la parte superior. En este aspecto, las 4 primeras variables muestran una homogeneidad en el resultado.
...
```{r 2.pregunta_12, echo=FALSE, cache=FALSE, results = 'asis', warning=FALSE, comment=FALSE}
titanic %>%
  mutate(Decade = cut(
    Age,
